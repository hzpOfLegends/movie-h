...
export default {
  data() {
    return {
      ...
      activeIndex: "0", 
      // 图片上传地址
      uploadURL: process.env.VUE_APP_IMG_URL,
      addForm: {
        goods_name: null,
        introduce: null,
        goods_price: null,
  ...
  methods: {
    ...
    // 获取商品分类数据列表
    async getCateList() {
      const { data: res } = await this.$http.get("category/data");
      if (res.status !== 200) {
    ...
    beforeTabLeave(activeName, odlActiveName) {
      // 未选中商品分类阻止Tab标签跳转
      if (odlActiveName === "0" && this.addForm.goods_cat.length !== 2) {
    ...
    handleSuccess(response) {
      // 1.拼接得到一个图片信息对象 临时路径
      const picInfo = { pic: response.data.filename };
